...
| 담당 | 영역 |
|---|---|
| 손민주 | 데이터 생성 · 마이데이터 어댑터 · RAG 파이프라인 |
| 박서희 | 최적화 엔진 · 데이터베이스 설계 |
| 조하영 | API 서버 · 인프라 · 시계열 예측 |

프론트엔드는 각자 담당한 백엔드 기능의 화면을 직접 구현한다.

- 손민주 — 페르소나 선택 및 데이터 업로드, 규칙 검수 화면
- 박서희 — 결제 라우팅 결과, 근거 약관 표시
- 조하영 — 가용잔고 대시보드, 시뮬레이션 입력, 잔고 추이 차트
