...
개인 한 명의 수개월치 데이터로 딥러닝 모델을 학습시키면 과적합이 발생하고 결과 설명도 어렵다. 카테고리별 이동평균과 중앙값으로 점 추정을 내고, 총액 표본의 분위수로 3개 시나리오 밴드를 산출한다. 밴드를 카테고리별 분위수의 합이 아니라 총액에서 구하는 이유는, 합산하면 모든 카테고리가 같은 달에 동시에 고점을 찍는다고 가정하게 되어 밴드가 실제보다 최대 3배 넓어지기 때문이다. 계절 효과는 통계적 유의성을 위해 최소 2년 치 데이터가 필요하므로 현 단계에서는 반영하지 않는다.
