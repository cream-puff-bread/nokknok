...
**Error responses**: every response — including 404 for an unknown path and 405 for a wrong method — returns `ErrorResponse {code, message}` from `src/api/errors.py`; FastAPI's default `{"detail": ...}` bodies are converted by handlers registered in `create_app()`. The `HTTPException` handler must be registered on **`starlette.exceptions.HTTPException`**, not `fastapi.HTTPException`: the router raises the parent class for 404/405, so registering the subclass silently misses them. `message` is user-facing copy — never put exception text or field names in it (`contracts/ui-system.md` forbids showing raw errors). `ErrorCode` must stay in sync with the `enum` in `contracts/api-spec.yaml`; `tests/test_api_errors.py` asserts this.
