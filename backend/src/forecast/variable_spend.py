...
2. **할부 거래** — 거래 금액은 구매가 전액이라 구매한 달만 비정상적으로
   커진다. 다만 표본에서 뺀다고 없는 돈이 되는 것은 아니다. 남은 회차는
   금액과 날짜가 이미 정해진 확정 지출이므로 projection 이 별도 스케줄로
   더한다(`_transaction_installment_schedule`).

## 정기 결제(is_recurring)를 빼지 않는 이유

처음에는 "fixed_expense 에 이미 있으니 빼야 한다"고 봤는데, 시드 데이터를
확인해 보니 사실이 아니었다. is_recurring 거래는 통신요금 자동이체뿐이고
fixed_expense 에는 통신 항목이 없다. 빼면 페르소나마다 월 49,000~52,000원이
예측에서 통째로 사라진다.

FileProvider 는 아예 fixed_expenses=[] 를 반환하므로 업로드 경로에서는
정기 지출 전부가 사라진다.

정기 결제는 규칙적이라 중앙값·이동평균이 오히려 잘 다룬다. 두 번 빼는 문제는
fixed_expense 와 거래가 겹칠 때만 생기는데, 겹치지 않는 것이 데이터의 불변식이다.
겹치기 시작하면 여기서 걸러야 한다.
...
크게 부풀려진다. 시연 데이터로 재보면 (q75-q25)/q50 이 총액 기준 0.12~0.17
인데 카테고리 합산 기준으로는 0.25~0.55 로 최대 3.4배가 됐다. 실제로는 한 달에
...
def _is_variable(txn: Transaction) -> bool:
    ...
    할부만 뺀다. 구매가 전액이 한 달에 잡혀 그 달 총액을 왜곡하기 때문이며,
    남은 회차는 projection 이 확정 지출로 따로 더한다.
    """
