...
- 그래프의 "위기" 표시가 **어디에 찍히는지**는 응답의 `deadPoint`(시나리오·달)를
  그대로 쓴다. 프론트에서 첫 적자 달을 다시 찾으면 적자 판정이 두 곳에 생겨,
  `forecast/projection.py` 의 `_find_dead_point` 와 어긋날 수 있다.
- 그 표시의 **색은** 위치와 별개로 안내 상자와 같은 기준(보통 시나리오가
  적자인가)을 쓴다. 점이 놓인 선의 색을 따르게 했더니, 보통 시나리오까지
  적자인 날에 굵은 선과 상자는 빨간데 적자 시점을 짚는 점만 호박색으로 남았다.
  가장 먼저 무너지는 시나리오와 그 결제가 얼마나 위험한지는 다른 이야기다.
